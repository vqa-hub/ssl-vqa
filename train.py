...
def compute_self_loss(logits_neg, a):
    #the loss of computing the relevance of image-question pairs
    ...
    qice_loss = torch.relu(neg_top_k).mean()
...
def train(model, train_loader, eval_loader, opt):
    ...
    scheduler = MultiStepLR(optim, milestones=[10,20,30,40,50,60], gamma=0.5)
